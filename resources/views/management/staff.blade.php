...
@section('content')
    <div class="content-container">
        <div class="item-container">
            @foreach($staff as $one_staff)
                <?php
                    $staff_class_name = str_replace(' ', '_', $one_staff->account->username);
                ...
                ?>
                <div id="staff-{{ $staff_class_name }}" class="company-row">
                    ...
                    <div class="right-container">
                        @if($one_staff->privileged)
                            <div class="edit-object center-content">
                                <img src="{{ asset('/images/crown.png') }}" alt="Staff privilegiato">
                            </div>
                        @endif
                        <div class="edit-object center-content">
                            <div class="button-container edit-button-container center-content">
                                <a href={{ route('staff.edit', [$one_staff]) }}>
                        ...
                        <div class="delete-object center-content">
                            <form action="{{ route('staff.destroy', $one_staff->id) }}" method="POST" onsubmit="deleteStaff(this, '{{ $one_staff->account->username }}')">
                                @csrf
                                @method('DELETE')
    ...
    <script>
        function deleteStaff(formElement, staffUsername){
            event.preventDefault();
            let staffClassName = staffUsername.replace(/ /g, "_");
            if (confirm("Sei sicuro di voler rimuovere " + staffUsername + " dallo staff?")) {
                sendDeleteAJAX({
                    url: $(formElement).attr("action"),
                    token: "{{ csrf_token() }}",
                    onSuccess: () => {
                        window.location.href = "{{ route('management.staff') }}"
                    }
                });
            }
        }
